...
class roleadd(commands.Cog):
    ...
    async def roleadd(self, ctx, member: str, *, role: str):
        ...
        if actual_role.position >= ctx.guild.me.top_role.position:
            return await self.bot.sendmessage(ctx,
                "i can't assign that role (higher than my top role)."
            )
        ...
        if ctx.author != ctx.guild.owner:
            ...
            if actual_role.position >= ctx.author.top_role.position:
                return await self.bot.sendmessage(ctx,
                    "you cannot assign a role equal or higher than your top role."
                )
        ...
        if actual_role in member.roles:
            return await self.bot.sendmessage(ctx,
                f"{member.mention} already has that role."
            )

        await member.add_roles(actual_role)

        await self.bot.sendmessage(ctx,
            f"added **{actual_role.name}** to {member.mention}."
        )
